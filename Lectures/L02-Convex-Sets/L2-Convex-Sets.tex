...
\begin{frame}{Linear Hull}
  ...
  The \emph{linear hull} $\operatorname{span} (S) \defn \cap_{L~\rm{linear},\, S\subseteq L} ~L$ of a set $S$ is the unique smallest linear set that contains $S$, i.e.\
  $$ L~\mathrm{linear~set}, ~S \subseteq L \implies \operatorname{span}(S) \subseteq L$$

  \vfill

  The \emph{dimension} $\dim(L)=k$ of a linear space $L$ is the smallest number of vectors $v_i\in L$ such that $$\operatorname{span}(\{v_1, \dots, v_k\}) = L.$$
...
\begin{frame}{Affine Sets}
  ...
  \item Solution set of linear system $\set{x\in\Re^n}{Ax = b}$ with $A\in \Re^{m\times n}$
  \end{itemize}
...
\begin{frame}{Conic Combinations}
  ...
  \textbf{Theorem : } A convex cone $K \subseteq E$ is closed under conic combinations.
...
\begin{frame}{Polyhedra}
  ...
  \item The probability simplex
    ...
    \]
  \item The convex hull of a finite set $\{x_1, \dots, x_k\}$ (Minkowski--Weyl theorem)
  \end{itemize}
...
\begin{frame}{Ellipsoid}
  ...
  \begin{align*}
    & \set{x}{\norm{x-x_c}_2^2 = (x-x_c)\tpose (x-x_c) \leq r^2} \\
    = & \set{x_c + r u}{\norm{u}_2 \leq 1}
  \end{align*}
  \vfill
  \emph{Ellipsoid} :
...
\begin{frame}{Elementary Convex Functions}
  ...
  \item Affine functions, e.g.\
    \[
      f(x) = a\tpose x - b.
    \]
    Convex on $\Re^n$ for any $a\in\Re^{n}$ and $b\in\Re$.
  ...
  \item<3-> Negative logarithm $f(x) = -\log(x)$. Convex on $\Re_{++}$.
...
\begin{frame}{Elementary Concave Functions}
  ...
  \item Affine functions, e.g.\
    \[
      f(x) = a\tpose x - b.
    \]
    Concave on $\Re^n$ for any $a\in\Re^{n}$ and $b\in\Re$.
...
\begin{frame}{Composition with Affine Functions}
  ...
  \textbf{Theorem : } Define the composition $h:E_1\to\nRe$ as
  \[
...
\begin{frame}{Partial Minimization}
  ...
  \vfill
  \uncover<2->{
  \textbf{Proof [$\star$]:} Take $y_1$, $y_2\in E_2$ and $\theta\in(0, 1)$ ($\theta\in\{0,1\}$ is trivial).

  \emph{Trivial case} ($g(y_1)=-\infty$, say): pick $x_2\in C$ with $f(x_2, y_2)<\infty$ (if none exists, $g(y_2)=+\infty$ and there is nothing to prove) and, for every $M>0$, an $x_1\in C$ with $f(x_1, y_1)\leq -M$. As $\theta x_1 + (1-\theta) x_2\in C$,
  \[
    g(\theta y_1 + (1-\theta) y_2) \leq \theta f(x_1, y_1) + (1-\theta) f(x_2, y_2) \leq -\theta M + (1-\theta) f(x_2, y_2)
  \]
  and letting $M\to\infty$ gives $g(\theta y_1 + (1-\theta) y_2) = -\infty$; the inequality holds.

  \emph{Interesting case} ($g(y_1), g(y_2)>-\infty$): for $\epsilon>0$ choose $x_1$, $x_2\in C$ with $f(x_i, y_i)\leq g(y_i)+\epsilon$ (any $x_i\in C$ works when $g(y_i)=+\infty$). As $\theta x_1 + (1-\theta) x_2\in C$ by convexity of $C$,
  \begin{align*}
    g(\theta y_1 + (1-\theta) y_2) & \leq f(\theta x_1 + (1-\theta) x_2, ~\theta y_1 + (1-\theta) y_2)\\
                                   & \leq \theta f(x_1, y_1) + (1-\theta) f(x_2, y_2) \leq \theta g(y_1) + (1-\theta) g(y_2) + \epsilon.
  \end{align*}
