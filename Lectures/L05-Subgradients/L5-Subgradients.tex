...
\begin{frame}{A Brief Recall of Differentiability}
  ...
  \item The function $f$ is \emph{directionally differentiable} at $x \in \dom(f)$ in the direction $d$ if the following limit exists:
    ...
    \]
  \item<2-> The function $f$ is (G\^ateaux) \emph{differentiable} in $x \in \dom(f)$ if directionally differentiable in $x$ in every direction $d \in \Re^n$ and if the directional derivative is linear in the direction, i.e.,
    \[
...
\begin{frame}{Gradient Method Converges with Fixed Step-Size}
  ...
  \textbf{Theorem:} Let $f$ be $L$-smooth w.r.t.\ $\norm{\cdot}_2$ and $-\infty < p^\star = \inf_{x\in\Re^n} f(x)$. The gradient method with step-size $0<h_k = \eta < \frac{2}{L}$ generates iterates with $\norm{f'(x_k)}\to 0$, i.e., every limit point of $(x_k)$ is stationary.
...
\begin{frame}{Optimal Fixed Step-Size}
  ...
  \begin{align*}
    \eta^\star = & \arg\max_{\eta} ~\eta(1-\tfrac{L \eta}{2})\\
    = & ~\frac{1}{L}.
  \end{align*}
...
\begin{frame}{Gradient Descent with Exact Line Search}
  ...
  \textbf{Theorem:} Let $f$ be $L$-smooth w.r.t.\ $\norm{\cdot}_2$ and $-\infty < p^\star = \inf_{x\in\Re^n} f(x)$. The gradient method with exact line search (assuming the minimizers $h_k$ exist) generates iterates with $\norm{f'(x_k)}\to 0$, i.e., every limit point of $(x_k)$ is stationary.
...
\begin{frame}{Existence of Proper Step Sizes}
  ...
  \textbf{Algorithms:} There are efficient algorithms to find points in $W_{\alpha, \beta}(\bar x)$.
\end{frame}

\begin{frame}{Proof [$\star$]}

  \textbf{Proof:} We have $g'(t) = -\iprod{f'(\bar x-t f'(\bar x))}{f'(\bar x)}$ and, as $f'(\bar x)\neq 0$, $g'(0)=-\norm{f'(\bar x)}^2<0$. Let $d(t)\defn g(t)-g(0)-\alpha t g'(0)$. Then, $d'(0)=g'(0)(1-\alpha)<0$.
  By continuity of $f'$ there exists a $T>0$ such that $d'(t)<0$ for all $t\in [0, T]$. By the \emph{MVT}, for any $t'\in(0, T]$ there is an $\bar t \in (0, t')$ such that
  \[
    d(t')-d(0) =  t' d'(\bar t) <0.
  \]
  Hence, $d(t')<d(0)$ for all $t'\in(0,T)$. Note that $g(t)$ is bounded from below for all $t> 0$ while $g(0)+\alpha t g'(0)$ with $\alpha>0$ is not. Hence, $d(t)\to+\infty$ and by continuity of $d$ the first zero
  \[
    t_a \defn \min \set{t>0}{d(t)=d(0)=0} \geq T
  \]
  exists.
  All step sizes $\bar t \in (0, t_a)$ evidently satisfy the sufficient decrease condition. The \emph{MVT} guarantees $d(t_a)-d(0)=t_a d'(t_b) = 0$ for some $t_b \in (0, t_a)$. Hence, $d'(t_b) = g'(t_b)-\alpha g'(0)=0$ and thus $g'(t_b)>\beta g'(0)$ as $\alpha < \beta$ and $g'(0)<0$. By continuity of $g'$, there is an $\epsilon>0$ such that all step sizes $\bar t \in (t_b-\epsilon, t_b+\epsilon)\subseteq(0, t_a)$ satisfy both Wolfe conditions.
...
\begin{frame}{Inexact Line Search}
  ...
  \textbf{Theorem:} Let $f$ be $L$-smooth w.r.t.\ $\norm{\cdot}_2$ and $-\infty < p^\star = \inf_{x\in\Re^n} f(x)$. The gradient method with any step size satisfying the Wolfe conditions generates iterates with $\norm{f'(x_k)}\to 0$, i.e., every limit point of $(x_k)$ is stationary.
...
\begin{frame}{Recall : Differentiable Convex Functions}

  Let $f:\Re^n\to\nRe$ be a differentiable convex function. For any two points $x, y\in\dom(f)$ we have
  \begin{align*}
    \iprod{f'(x)}{y-x} & = \lim_{t\downarrow 0} \frac{f(x + t(y-x))-f(x)}{t} \\
                       & \leq \lim_{t\downarrow 0} \frac{(1-t)f(x) + t f(y)-f(x)}{t} = f(y) - f(x).
  \end{align*}
...
\begin{frame}{Supporting Hyperplanes}
  ...
  A \emph{supporting hyperplane} $H=\set{x}{h(x)=\alpha}$, with $h\neq 0$ a linear function, of a convex set $S$ at a point $x_0 \in \bnd(S)$ is such that $x_0\in H$ and $S\subseteq H^-\defn \set{x}{h(x)\leq \alpha}$.
...
\begin{frame}{The Subdifferential is Nonempty}
  ...
  \]
  and $H$ is hence a supporting hyperplane of $\epi(f)$ at $(x, f(x))$
...
\begin{frame}{The Conjugate Function}
  ...
  We define the \emph{conjugate} of any function $f:\Re^n\to\nRe$ as
  \[
...
\begin{frame}{Examples}
  ...
  \item Polytope : $X=\conv(\{v_0, \dots, v_k\})$.
    \[
    ...
    \]
  \item Half-space : $X= \set{x\in \Re^n}{\iprod{a}{x}\leq b}$ with $a\neq 0$. Then, $\sigma_X(\gamma a) = \gamma b$ for $\gamma \geq 0$ and $+\infty$ otherwise.
...
\begin{frame}{Support Function of Subdifferential}
  ...
  Let $f:\Re^n\to\nRe$ be a \ac{lsc} convex function. Let $x\in\interior(\dom(f))$.
...
\begin{frame}{Proof}
  ...
  Fix $x\in \interior(\dom(f))$ and $d\in \Re^n$. We need to check $h(d) = \sup \set{\iprod{g}{d}\!}{\!g \in \partial f(x)}$. First, $h(d) \geq \sup \set{\iprod{g}{d}\!}{\!g \in \partial f(x)}$ as
  \[
    \forall g \in \partial f(x), ~ h(d)\defn \lim_{t\downarrow 0} \, \mu(t)\defn (f(x+t d) - f(x))/t \geq \iprod{g}{td}/t = \iprod{g}{d}.
  \]
  Second, the function $h$ is convex with $\dom(h)=\Re^n$ as $f$ is convex and $x\in\interior(\dom(f))$. Thus, $\partial h(d) \neq \emptyset$. Let $g_h\in \partial h(d)$ and $v\in\Re^n$. Note that for all $\tau \geq 0$
  \[
  ...
  \[
    f(y) - f(x) \geq \lim_{t\downarrow 0} \, (f(x + t(y-x))-f(x))/t =  h(y-x) \geq \iprod{g_h}{y-x} \quad \forall y \in \dom(f).
  \]
...
\begin{frame}{Subdifferential and Differential}
  ...
  \textbf{Theorem :} A \ac{lsc} convex function $f$ is differentiable at $x\in\interior(\dom(f))$ if and only if $\partial f(x) = \{g\}$. In that case, $g = f'(x)$.
