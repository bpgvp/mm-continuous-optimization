...
\begin{tikzpicture}
  ...
  \begin{axis}[
    ...
    \node[right] at (axis cs:0.3, 0.8) {\tiny \red{$f(x_k) + \iprod{f'(x_k)}{x-x_k}$}};

    \node[circle, inner sep=1, fill=red] (x) at (axis cs:0.2, 0.771101) {};
    \node[above] at (x) {\tiny \red{$x_k$}};

    \node[circle, inner sep=1, fill=red] (y) at (axis cs:1.5, 3.00248) {};
    \node[above] at (y) {\tiny \red{$x$}};

    \node (y_a) at (axis cs:1.5, 1.758968) {};
