...
\begin{frame}{Playing with Constraints}
  ...
  Let $\mc F$ be a set of \emph{non-decreasing (price) functions} $F:\Re^m\to\Re\cup\{\pm\infty\}$. That is, 
  \[
    u \succeq_{\Re^m_+} v \implies F(u) \geq F(v).
  \]
  We denote the set of \emph{all} such non-decreasing functions by $\mc F(\Re^m_+)$.
...
\begin{frame}{Example}
  ...
  \[
    \begin{array}{rl}
      p^\star=\minimize & f(x) \defn x_1 + x_2 + \dots + x_{3000}\\[0.5em]
      \st & g_1(x) = x_1 + 2 x_2 + \dots + 3000 x_{3000}\geq 1 = b_1 \\[0.5em]
                & g_2(x)=3000 x_1 + 2999 x_2 + \dots + x_{3000} \geq 100 = b_2
    \end{array}
  \]

  \textbf{Remark:}
  \begin{itemize}
  \item Consider the price function $F^\star= (c_1, c_2) \mapsto (c_1+c_2)/3001\in \mc F(\Re^2_+)$
  \item<2-> Clearly
    \begin{align*}
      & F^\star(g(x)) = F^\star(x_1 + 2 x_2 + \dots + 3000 x_{3000}, 3000 x_1 + 2999 x_2 + \dots + x_{3000})\\
      = & x_1 + x_2 + \dots + x_{3000} = f(x)
    \end{align*}
  \item<3-> Hence, $p^\star\geq F^\star(b) = 101/3001$
  \end{itemize}
...
\begin{frame}{Generalized Conic Constraints}
  ...
  The dual is built from a set $\mc F$ of \emph{non-decreasing (price) functions} $F:\Re^m\to\Re\cup\{\pm\infty\}$, now with respect to the cone $K$:
  \[
    u \succeq_{K} v \implies F(u)\geq F(v).
  \]
  ...
  \[
    \begin{array}{rlcrl}
      ...
      \st & x\in X   & &                       \st & F \in \mc F(\Re^m_+) \\[0.5em]
           & g(x) \succeq_{\Re^m_+} b &&    & F(g(x)) \leq f(x), ~~\forall x \in X.
    \end{array}
  \]

  Recall that $f(x) \geq F(g(x)) \geq F(b)$ for all primal feasible $x$ and dual feasible $F$.


  \vfill
  \textbf{Remarks :}
  \begin{itemize}
  \item<2-> In this general setting strong duality $p^\star=d^\star$ always holds.
  \end{itemize}
...
\begin{frame}{Strong Duality}
  ...
  \textbf{Proof : }
  \begin{itemize}
  \item<2-> Define the \emph{perturbation function} as follows
    \[
      \begin{array}{rl}
        \phi(b) \defn \inf_x & f(x) \\
        \st & x\in X\\
                             & g(x) \succeq_{\Re^m_+} b.
      \end{array}
    \]
  \item<3-> Observe that $\phi \in \mc F(\Re^m_+)$ and that $\phi(g(x))\leq f(x)$ for all $x\in X$
  \item<4-> Hence,
    \[
      \begin{array}{rll}
        \phi(b) \geq \sup_F & F(b) & \geq \phi(b) \\[0.5em]
...
\begin{frame}{General Strong Duality}
  ...
  \uncover<2->{
    Instead we will consider a subclass $\mc F \subset \mc F(\Re^m_+)$.
    \begin{itemize}
...
\begin{frame}{Affine Dual}

  Nondecreasing affine functions $\mc F_{{\rm{aff}}} = \set{c\mapsto \iprod{u}{c} + u_0}{u\in \Re_+^m,~u_0\in \Re}$ are a sensible choice.
%
  Dual problem becomes
  \[
    \begin{array}{rl}
      d^\star(\mc F_{{\rm{aff}}}) = \sup  & F(b) \\[0.5em]
                       \st   & F\in \mc F_{{\rm{aff}}} \\[0.5em]
                             & F(g(x)) \leq f(x),~~ \forall x \in X.
    \end{array}
  \]
  ...
  We know that if perturbation function $\phi\in \mc F_{{\rm{aff}}}$ then we have strong duality.
  ...
  \uncover<2->{
    \textbf{Remarks:}
    \begin{itemize}
    \item Even in linear optimization the perturbation function is (convex) piece-wise linear rather than affine!
    \item Hence, we expect $\phi \not \in \mc F_{{\rm{aff}}}$ which is problematic
    \end{itemize}
  }  
\end{frame}


\begin{frame}{When We Do Not Lose Anything}
  ...
  \textbf{Theorem : } If perturbation function $\phi  = \sup_{F\in \mc S}\, F $ for $\mc S \subseteq \mc F$, then $p^\star=d^\star(\mc F)$.

  \vfill

  \uncover<2->{
  \textbf{Proof : } Since $F(g(x)) \leq \phi(g(x)) \leq f(x)$ when $x\in X$ for all $F\in \mc S$,
  \[
    \begin{array}{rlr}
      \phi(b) \defn \inf_x & f(x) & \geq d^\star(\mc F) \geq \sup_{F\in \mc S}\,F(b) = \phi(b).\\
      \st & x\in X & \\
                         & g(x) \succeq b. & 
    \end{array}
  \]
...
\begin{frame}{Geometrical Interpretation}
  ...
  \[
    \mc F_{{\rm{aff}}} =\set{c\mapsto\iprod{u}{c}+u_0}{u\in\Re_+^m, ~u_0\in \Re}.
  \]
  However, $\phi \not\in \mc F$. Nevertheless, $\phi=\max_{F\in \mc S} F$ for some $\mc S\subseteq \mc F_{{\rm{aff}}}$.
...
\begin{frame}{Global Sensitivity Interpretation}
        ...
        {
          {
            ...
            {exp(0.3*\p) + 0.3*exp(0.3*\p)*(x-\p)};
            \addlegendentry{$F^\star$}
          }
        };

        \draw[densely dotted] (axis cs:2,0)--(axis cs:2,1.82212);
        \node[above right] at (axis cs:2,0.7) {$b_0$};

        \draw[densely dotted] (axis cs:0,1.82212)--(axis cs:2,1.82212);
        \node[above right] at (axis cs:0,1.82212) {$\phi(b_0) = F^\star(b_0)$};
  ...
  \textbf{Remark:}
  \begin{itemize}
  \item For any dual feasible $F$ we have $\phi(b)\geq F(b)$ for all $b$.
  \item<2-> Consider a dual optimal $F^\star$ at $b_0$ with $F^\star(b_0)=\phi(b_0)$.
  \item<3-> The optimal dual function $F^\star=c\mapsto  \iprod{u^\star}{c-b_0}+\phi(b_0)$ serves as a \emph{sensitivity/price} function of the primal infimum.
  \end{itemize}  
...
\begin{frame}{Example}
  ...
  \begin{itemize}
  \item Consider the price function $F^\star= (c_1, c_2) \mapsto (c_1+c_2)/3001\in \mc F(\Re^2_+)$
  \item<2-> Suppose we are forced to guarantee $g_1(x)\geq 1+\epsilon$. Additional cost will scale as
    \[
      p^\star \approx 101/3001+\epsilon/3001
    \]
    for small $\epsilon>0$.
  \end{itemize}
...
\begin{frame}{Lower Semi-Continuous Functions}
  ...
  \textbf{Definition :} A \emph{lower semi-continuous} (LSC) function is a function with closed epigraph. A function is \emph{proper} if it never takes the value $-\infty$ and is not identically $+\infty$.
  
  \uncover<2->{
  \textbf{Theorem :} Every proper convex LSC function is the supremum of affine functions.
...
\begin{frame}{Strong Duality in Nonlinear Programming}
  ...
  \textbf{Theorem :} If the perturbation function $\phi$ is proper, convex and LSC, then strong duality holds $p^\star=d^\star(\mc{F}_{{\rm{aff}}})$.
  ...
  \item \emph{Dual is always a convex problem}
    ...
    \[
      \begin{array}{rl}
        d^\star(\mc{F}_{{\rm{aff}}}) = \sup & \iprod{u}{b} + u_0 \\[0.5em]
        \st & \iprod{u}{g(x)}+ u_0 \leq f(x), ~~\forall x\in X \\[0.5em]
                                 & u\geq 0
      \end{array}
    \]
  \item<2-> Often showing ``$\phi$ is LSC and convex'' is not too difficult
  \end{itemize}
\end{frame}
...
%       \node[rectangle, draw=black, below=1cm of strong_duality_partial] (lsc) {$\mc F_{{\rm{aff}}}=\{\rm{affine}\}$, $\phi$ convex LSC};
...
\begin{frame}{Strong Duality -- Summary}
  ...
  and $\mc F$ sufficiently rich, i.e.\
  \begin{enumerate}
  \item $c\mapsto\iprod{u}{c}$ belongs to $\mc F$ for all $u$ in $K^\star$.
  \item $F\in \mc F$ and $\alpha\geq 0$ implies $\alpha F \in \mc F$.
  \item $F\in \mc F$ and $a\in \Re$ implies $a+F \in \mc F$
  \end{enumerate}
  ...
  \uncover<2->{
  In particular, $\mc F_{\rm{aff}}\defn \set{c\mapsto \iprod{u}{c} + u_0}{u\in K^\star}$ satisfies all conditions.
  }
\end{frame}
...
\begin{frame}{Strong Duality -- Summary (Affine Functions)}
  ...
  \vfill
  For $K=\Re^m_+$ and $\mc F_{{\rm{aff}}}\defn \set{c\mapsto \iprod{{u}}{c} + u_0}{{u}\geq 0}$, the Lagrangian simplifies to \[L(x, {u}) = f(x) + \iprod{{u}}{b-g(x)}.\]
...
%   \]
%   \textbf{Proof:}
%   We have strong duality unconditionally for linear optimization problems which are feasible and bounded (a consequence of Farkas' lemma).
...
\begin{frame}{SVM : Primal Formulation}
  ...
  \item \emph{Classifier} $h(x) = \operatorname{sign}(x\tpose w + w_0)$
  \end{itemize}
...
\begin{frame}{Nonlinear Embedding}
  ...
  \begin{minipage}{0.6\textwidth}
    ...
    $\varphi(x) = (1, x_1, x_2, x_1^2, x_1 x_2, x_2^2, \dots, x_p^2 )$\\[5em]
  \end{minipage}
  ...
  % \item Classifier $h(x) = \operatorname{sign}(\iprod{\emph{\varphi}(x_i)}{w} + w_0)$
  % \end{itemize}
...
\begin{frame}{Special Case}
  ...
  \)
  As $u_i^\star\geq 0$ and $g_i(x^\star)\geq b_i$, it follows that $$u_i^\star \left(g_i(x^\star)-b_i\right)=0 \quad \forall i\in [1,\dots, m].$$
...
\begin{frame}{Proof}
  ...
  \textbf{Proof [$\star$]:}
  (I) $x^\star\in \arg\min_{x\in X} ~L(x, F^\star)$. As $F^\star$ is dual feasible, $F^\star(g(x))\leq f(x)$ and hence
  \[
    L(x, F^\star) = f(x) - F^\star(g(x)) + F^\star(b) \geq F^\star(b) = f(x^\star) \quad \forall x\in X.
  \]
  From the previous theorem $F^\star(g(x^\star)) = F^\star(b)$ and thus $L(x^\star, F^\star) = f(x^\star)$. Hence, $x^\star$ attains the lower bound.

  \vfill

  (II) $F^\star \in \arg \max_{F\in \mc F} ~L(x^\star, F)$. As $x^\star$ is primal feasible, $F(g(x^\star))\geq F(b)$ and hence
  \[
    L(x^\star, F) = f(x^\star) + F(b) - F(g(x^\star)) \leq f(x^\star) \quad \forall F\in \mc F.
  \]
  By (I), $L(x^\star, F^\star) = f(x^\star)$. Hence, $F^\star$ attains the upper bound.
...
\begin{frame}{Nonlinear Classification}
  ...
  \item \textbf{Classifier $h(x) = \operatorname{sign}(\emph{\varphi}(x)\tpose w + w_0)$}
  \end{itemize}
...
\begin{frame}{From Dual Solution to Primal Classifier}
  ...
  \emph{Classifier :} $h(x) = \operatorname{sign}(\sum_{j=1}^n u^\star_j y_j K(x, x_j) + w^\star_0)$ with $K(x, x_j)=\iprod{\varphi(x)}{\varphi(x_j)}$. 
  ...
  \[
    \begin{array}{rl}
      ...
      \implies & \sum_{i=1}^n u^\star_i  [\sum_{j=1}^n  u^\star_j y_j K(x_i, x_j) + w^\star_0-y_i] = 0\\[0.5em]
          \implies &      w^\star_0 = -\sum_{i=1}^n u^\star_i \sum_{j=1}^n u^\star_j y_j K(x_i, x_j)/(\sum_{i=1}^n u_i^\star).
    \end{array}
  \]
...
\begin{frame}{Slater's Condition}
  ...
  \textbf{Theorem : } Under convexity and Slater's condition (proof in the KKT lecture)
  \[
...
\begin{frame}{SVM : Constraint Qualification}
  ...
  \[
    ...
    \end{array}
  \]
  with $Q_{ij}=y_iy_j\iprod{x_i}{x_j}$ for all $1\leq i, j\leq n$.
